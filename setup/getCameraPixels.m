function [az,el,totalpixels] = getCameraPixels(type,fov)

% input "type" can be either "camera" or "auger"

if strcmp(type,'camera'),
    
    az0 = -fov.leftright/2 * pi/180;
    az1 = fov.leftright/2 * pi/180;
    el0 = (fov.elevation - fov.updown/2) * pi/180;
    el1 = (fov.elevation + fov.updown/2) * pi/180;
    [az,el] = meshgrid(linspace(az0,az1,fov.numpixels(1)),linspace(el0,el1,fov.numpixels(2)));
    
    totalpixels = fov.numpixels(1)*fov.numpixels(2);
    
    az = reshape(az,[],1);
    el = reshape(el,[],1);
    
elseif strcmp(type,'auger'),
    
    az = [13.51, 14.27, 13.54, 14.33, 13.61, 14.41, 13.70, 14.52, 13.82, 14.66;
    ...
    el = [ 2.08,  3.34,  4.61,  5.85,  7.14,  8.37,  9.66, 10.89, 12.18, 13.40;
    ...
    az = pi/180 * reshape(az,[],1);
    el = pi/180 * reshape(el,[],1);
    
    totalpixels = length(az);
