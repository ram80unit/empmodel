...
int main()
{
  ...
  fread(&pecground,sizeof(int),1,inputFile);
  ...
  double Isource [tsteps];
  fread(&Isource,sizeof(double),tsteps,inputFile);
  fread(&txf0,sizeof(double),1,inputFile);
  w0 = 2*PI*txf0;
  fread(&sourcealt,sizeof(double),1,inputFile);
  fread(&rsspeed,sizeof(double),1,inputFile);
  fread(&sourcetype,sizeof(int),1,inputFile);
  fread(&numfiles,sizeof(int),1,inputFile);
  fread(&venus,sizeof(int),1,inputFile);
  ...
  logfile << "Lightning located at altitude " << sourcealt/1000 << "\n";
  ...
  // source delays
  int tJs;
  int nextra = 5; // this is the "corona" around the source (for numerical reasons)

  int rsdelay[rr];
  int tcsdelay[rr];
  for (int i = 0; i < rr; i++) {
    rsdelay[i] = round((r[i]-RE)/rsspeed/dt);
    tcsdelay[i] = round((r[i]-RE)/C/dt);
  }

  // initialize source function
  int satop = nground + floor(sourcealt/dr1) - 1;
  // DATATYPE Js = 0;
  DATATYPE Jsv [satop+nextra];
  DATATYPE rssigma = 2000;  // altitude lambda for exponential decay (MTLE) 
  DATATYPE taud = 5e-6; // decay constant for DU source
  DATATYPE vstar = rsspeed/(1+rsspeed/C);
  int tcsind;
  int maxdelay = rsdelay[satop+nextra];
...
#pragma omp parallel for
    ...
    // source
    for (int i = nground; i < satop; i++) {

      ifl = (DATATYPE)i;
      tJs = t + maxdelay - rsdelay[i];
      if (tJs > tsteps) tJs = tsteps;
	...
	for (int k = sp-nextra; k < sp+nextra; k++) {
	  ...
	  if (sourcetype == 0) {
	    // TL source, no decay
	    Jsv[i] = Isource[tJs] * exp(-jfl*jfl/9.0) * exp(-kfl*kfl/9.0) / (PI * 9.0*drange*drange);
	  } else if (sourcetype == 1) {
	    // linear decay
	    Jsv[i] = Isource[tJs] * exp(-jfl*jfl/9.0) * exp(-kfl*kfl/9.0) / (PI * 9.0*drange*drange) * (1 - (r[i]-RE)/sourcealt);
	  } else if (sourcetype == 2) {
	    // exponential decay
	    Jsv[i] = Isource[tJs] * exp(-jfl*jfl/9.0) * exp(-kfl*kfl/9.0) / (PI * 9.0*drange*drange) * exp(-(r[i]-RE)/rssigma);
	  } else if (sourcetype == 3) {
	    // Bruce-Golde
	    if (t > rsdelay[i]) {
	      Jsv[i] = Isource[t] * exp(-jfl*jfl/9.0) * exp(-kfl*kfl/9.0) / (PI * 9.0*drange*drange);
	    }
	    ...
	    // Diendorfer-Umran (DU)
	    tJs = t + tcsdelay[i];
	    tcsind = round(i*dr1/vstar/dt);
	    if (t > rsdelay[i]) {
	      Jsv[i] = (Isource[tJs] - exp(-(t*dt-i*dr1/rsspeed)/taud)*Isource[tcsind]) * exp(-jfl*jfl/9.0) * exp(-kfl*kfl/9.0) / (PI * 9.0*drange*drange);
	      if (Jsv[i] < 0) Jsv[i] = 0;
	    }
	  } else if (sourcetype == 6) {
	    // dumb source, constant current
	    Jsv[i] = Isource[t] * exp(-jfl*jfl/9.0) * exp(-kfl*kfl/9.0) / (PI * 9.0*drange*drange);
	  } else {
	    Jsv[i] = 0;
	  }
	  if (dotransmitter) { // transmitter should look like small monopole: linear decay with altitude
	    Jsv[i] = Isource[t] * exp(-jfl*jfl/9.0) * exp(-kfl*kfl/9.0) / (PI * 9.0*drange*drange) * (1 - (r[i]-RE)/sourcealt);
	  }
	  Er[i][j][k] = Er[i][j][k] - c2e[i] * Jsv[i];
	}
      }
    }
    // smooth the top of the current for a few cells
    for (int i = satop; i < satop+nextra; i++) {
      ifl = (DATATYPE)(i-satop+1);
      for (int j = st-nextra; j < st+nextra; j++) {
	for (int k = sp-nextra; k < sp+nextra; k++) {
	  Jsv[i] = Jsv[satop] * exp(-ifl*ifl/nextra/nextra);
	  Er[i][j][k] = Er[i][j][k] - c2e[i] * Jsv[i];
	}
      }
    }
...
#pragma omp parallel for
	...
	for (int k = 1; k < pp-1; k++) {
	  ...
	  if (pecground) Et[nground-1][j][k] = 0;
	}
      }
    }
...
#pragma omp parallel for
	...
	for (int k = 0; k < pp-1; k++) {
	  ...
	  if (pecground) Ep[nground-1][j][k] = 0;
	}
      }
    }
	...
	for (int j = 0; j < hh; j++) {
	  for (int k = 0; k < pp; k++) {
	    ...
	    if (venus) {
	      ...
	      va = nd3[i][j][k]/nd[nground] * pow(10, (-0.2212 * EoEk*EoEk + 10.89*EoEk - 1.963)/(EoEk - 0.06729) );
	      vi = nd3[i][j][k]/nd[nground] * pow(10, (14.3 * EoEk*EoEk + 127.9*EoEk - 39.86)/(EoEk*EoEk + 10.85*EoEk - 0.9375) );
	      vd = 0;
	      vN21P = 0;
	      vN22P = 0;
	      vN2P1N = 0;
	      ...
	      tauN22P = 1/(2e7 + 3e-16*(0.00*nd3[i][j][k]) );  // quenching through O2, which is 0% of density
	      tauN21P = 1/(1.7e5 + 1e-17*(0.035*nd3[i][j][k]) ); // quenching through N2, 3.5%
	      tauN2P1N = 1/(1.4e7 + 4e-16*(0.035*nd3[i][j][k]) ); // quenched by N2 and O2
	      tauN2PM = 1/(7e4 + 5e-16*(0.035*nd3[i][j][k]) ); // quenched by N2
	      tauO2P1N = 1/(8.3e5 + 2e-16*(0.035*nd3[i][j][k]) ); // quenched by N2
